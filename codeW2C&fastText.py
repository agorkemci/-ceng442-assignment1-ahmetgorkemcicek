...
def process_file(in_path, text_col, label_col, scheme, out_two_col_path, remove_stopwords=False):
    ...
    #After the word of "üçün", I have proposed 10 words in azeri turkish by looking English stopwords .
    if remove_stopwords:
        sw = set(["və","ilə","amma","ancaq","lakin","ya","həm","ki","bu","bir","o","biz","siz","mən","sən","orada","burada","bütün","hər","artıq","çox","az","ən","də","da","üçün",
                  "hansı","əgər","bəlkə","yaxud","çünki","qədər","necə","bəzi","başqa","sadəcə"])
...
if __name__ == "__main__":
    ...
    w2v = Word2Vec(sentences=sentences, vector_size=300, window=5, min_count=3, sg=1, negative=10, epochs=10, workers=1, seed=SEED)
    w2v.save("embeddings/word2vec.model")

    ft = FastText(sentences=sentences, vector_size=300, window=5, min_count=3, sg=1, min_n=3, max_n=6, epochs=10, workers=1,seed=SEED)
    ...
    print(f"The no. of deasciified words: {slang_map_counter}")
